...
namespace AsyncLaunch
{
    void Async_Default()
    {
        ...
        LOG << "Before sleep." << std::endl;
        std::this_thread::sleep_for(std::chrono::seconds(5u));
        LOG << "Sleep ended." << std::endl;

        LOG << "Waiting for the second Thread..." << std::endl;
        asyncDefault.get();
        LOG << "Second done." << std::endl;
    }
    ...
    void Asynch_vs_Defered_2()
    {
        ...
        LOG << "with launch::deferred:" << std::endl;
    ...
    void Asynch_vs_Defered_3()
    {
        auto task = [](unsigned int timeout, const std::string& name)-> void {
            LOG << name << " entered" << std::endl;
            std::this_thread::sleep_for(std::chrono::seconds(timeout));
            std::cout << std::endl;
            LOG << name << " done." << std::endl;
        };

        LOG << "Starting async task ....." << std::endl;
        ...
        LOG << "Async task done. Deffered task should start" << std::endl;
        deferred_task.get();
        LOG << "We've done." << std::endl;
    }
}
...
namespace AsyncLaunch::Limiting_Threads_Number
{
    ...
    void RunTasks()
    {
        ...
        LOG << "Allowing only " << max_concurrent_tasks
...
namespace AsyncLaunch::TasksChaining
{
    int stage1(int x)
    {
        LOG << __FUNCTION__ << " | " << x << " -> " << x + 1 << std::endl;
        std::this_thread::sleep_for(std::chrono::milliseconds (250U));
        return x + 1;
    }

    int stage2(int x) {
        LOG << __FUNCTION__ << " | " << x << " -> " << x * 10 << std::endl;
        std::this_thread::sleep_for(std::chrono::milliseconds (250U));
        return x * 10;
    }
    ...
    void RunDependentTasks()
    {
        ...
        fut3.wait();
        LOG << "Final result: " << fut3.get() << std::endl;
    }
}
