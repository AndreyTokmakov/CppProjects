...
```
In the code above, one might think b is copied to the caller. <br>
...
C++17 mandates copy elision in certain cases. <br>
...
```
                                                                   Big g() {
                                                                       Big x;
Big f() {                                                              // ...
    return Big(); // guaranteed RVO in C++17, no copy at all           if(condition) {
...
prove one construction can be used for both).<br>
...
```
The compiler cannot apply NRVO here because two different local objects might be returned.<br>
It will likely do a move (C++11 onward) which is cheaper than copy, but if Big was not movable, it might copy. <br>
