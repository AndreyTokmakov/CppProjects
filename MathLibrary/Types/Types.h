...
namespace Types
{
    template<size_t _Size, typename Ty>
    class Point {
    private:
        using value_type = Ty;
    ...
    public:
        ...
        Point(std::initializer_list<value_type> list) {
            static_assert(0 != Capacity, "Number of coordinates should not be equal zero");
        ...
        */
        ...
        Point(Args&& ... params): coordinates {
            static_cast<value_type>(std::forward<Args>(params)) ...} {
        }
    ...
    public:
        ...
        [[nodiscard]]
        std::array<value_type, _Size>& asArray() noexcept {
            return coordinates;
        }

        [[nodiscard]]
        const std::array<value_type, _Size>& asArray() const noexcept {
            return coordinates;
        }
