...
namespace enums::EnumNewStyle {

	void EnumsStrictType_Test()
	{
		enum class Options { None, One, All };

		[[maybe_unused]]
		constexpr Options opts = Options::All;
	}

	void EnumsStrictType_ToInt()
	{
...
namespace enums::EnumClassTests {
...
namespace enums::Underlying_Type {
	...
	void Uniform_Initialization_Tests() {
		...
		[[maybe_unused]]
		constexpr AccountNumber account1 = AccountNumber(817986000); //OK, but dangerous

		[[maybe_unused]]
		constexpr Byte b0 {}; // OK. Value-Initialized with 0

		[[maybe_unused]]
		constexpr Byte b1 { 1 }; // OK

		[[maybe_unused]]
		constexpr Byte b3 = Byte{ 3 }; // OK
...
namespace enums::UsingEmum_ClassScope
{
    enum struct Color {
        red,
        green,
    ...
    {
    	[[maybe_unused]]
	    constexpr Something s;
        const Color _ = s.red;
    }
}

namespace enums::EnumConversationTest
{
    enum class Color {
        red,
        green,
...
namespace enums::Is_Scoped_Enum
{
    enum RGB
    {
        Red,
...
namespace enums::Consteval_SwitchCase_Bitmask
{
...
namespace enums::Check_Order_Type_without_OR
{
...
namespace enums::Check_match_with_one_expression
{
	enum class Weekdays
	{
		Monday = 1,
	...
	constexpr bool isWorkingDay(const Weekdays day)
	{
		...
		static constexpr int32_t workdayMask = static_cast<int>(Monday) | static_cast<int>(Tuesday) |
			static_cast<int>(Wednesday) |static_cast<int>(Thursday) | static_cast<int>(Friday);

		return (static_cast<int>(day) & workdayMask) != 0;
	}

	void Test()
	{
		static_assert(isWorkingDay(Weekdays::Monday));
		static_assert(isWorkingDay(Weekdays::Tuesday));
		static_assert(isWorkingDay(Weekdays::Wednesday));
		static_assert(isWorkingDay(Weekdays::Thursday));
		static_assert(isWorkingDay(Weekdays::Friday));
		static_assert(!isWorkingDay(Weekdays::Saturday));
		static_assert(!isWorkingDay(Weekdays::Sunday));
	}
}
...
void enums::TestAll()
{
	...
	// EnumNewStyle::EnumsStrictType_Test();
	...
	Check_match_with_one_expression::Test();
};
