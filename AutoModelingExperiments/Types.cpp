...
namespace Types::LineTesting
{
    ...
    {
        const Point<3,double> prFrom {2.5, 4.0, 1.5};
        const Point<3,double> B {0, 0, 0}, C {16.0, 0, -10.3};

        // Calc3DLineEquation({pt1, pt2});

        const Point<3,double> d = (C - B) / C.distanceTo(B);
        const Point<3,double> v = prFrom - B;
        double t = Dot(v, d);
        const Point<3,double> P = B + d * t;

        std::cout << P << std::endl;
        std::cout << "Distance = " << P.distanceTo(prFrom) << std::endl;
    }
}
