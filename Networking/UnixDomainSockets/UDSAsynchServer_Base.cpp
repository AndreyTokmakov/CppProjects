/**============================================================================
Name        : UDSAsynchServer_Base.cpp
Created on  : 31.03.2025
Author      : Andrei Tokmakov
Version     : 1.0
Copyright   : Your copyright notice
Description : UDSAsynchServer_Base.cpp
============================================================================**/
...
namespace UDSAsynchServer_Base
{
    struct UDSServer
    {
...
namespace UDSAsynchServer_Base
{
    ...
    bool UDSServer::start()
    {
        ...
        message.reserve(maxReceiveSize * 10);
        std::array<unsigned char, maxReceiveSize> buffer {};
        ...
        while (true)
        {
            ...
            for (uint32_t idx = 0; idx < currentSize; idx++)
            {
                ...
                if (0 == pollEvent.revents)
                    continue;
                if (Socket clientSocket { INVALID_HANDLE }; pollEvent.revents != POLLIN)
                {
                ...
                {
                    clientSocket = pollEvent.fd;
                    while (true)
                    {
                        const int64_t bytesRead = ::recv(clientSocket,buffer.data(), maxReceiveSize, 0);
                        ...
                        message.insert(message.end(), buffer.data(), buffer.data() + bytesRead);
                        if (maxReceiveSize > bytesRead)
                        {
                            std::cout << std::string_view(reinterpret_cast<const char *>(
                                 message.data()), message.size())<< std::endl;
                            message.clear();
                            break;
                        }
                    }
...
void UDSAsynchServer::runServer_Base()
{
    using namespace UDSAsynchServer_Base;
