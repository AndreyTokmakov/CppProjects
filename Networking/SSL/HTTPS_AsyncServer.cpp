...
namespace HTTP_AsyncServer
{
    ...
    class TCPServer
    {
        ...
        bool getClientRequestHandle(handleType& clientHandle,
...
namespace HTTPS_AsyncServer
{
    ...
    class TCPServer
    {
        ...
        {
            handleType clientSocket { INVALID_SOCKET };
            std::shared_ptr<SSL> ssl {nullptr};
        };
        ...
        template<class Rep, class Period>
        bool getClientRequestHandle(Context& clientHandle,
                                    const std::chrono::duration<Rep, Period> &timeout) noexcept
        {
            {
                std::unique_lock<std::mutex> lock {mutex};
        ...
        void processRequest()
        {
            ...
            while (run)
            {
                if (bool result = getClientRequestHandle(context, QUEUE_TIMEOUT); result)
                {

                    if (const int bytesRead = SSL_read(context.ssl.get(), buffer.data(), bytesToRead);
                    ...
                    if (const int bytesWritten = SSL_write(context.ssl.get(), response.data(), response.length());
                            bytesWritten) {
        ...
        // TODO: Rename
        void submit(handleType clientSocketHandle) noexcept
        {
            Context context {
            ...
            if (const int result = SSL_set_fd(context.ssl.get(), clientSocketHandle); 1 != result)
            {
            ...
            if (const int result = SSL_accept(context.ssl.get()); 1 != result)
            {
            ...
            {
                std::lock_guard<std::mutex> lock(mutex);
                queue.push_back(std::move(context));
            }
            gotClientRequest.notify_one();
        }
        ...
        handleType closeClientConnection(handleType handle) const
        {
            // TODO: Need to find ClientSocket Context { sock, SSL } and close
    ...
    public:
        ...
        TCPServer(std::string address, uint16_t port):
                ...
                listenPort {port},
                ctx {std::unique_ptr<SSL_CTX, decltype(&::SSL_CTX_free)> {nullptr, SSL_CTX_free }} {
            try {
                SSL_library_init();
                ...
                ctx = std::unique_ptr<SSL_CTX, decltype(&::SSL_CTX_free)> {SSL_CTX_new(method), SSL_CTX_free };
                if (!ctx) {
        ...
        {
            // TODO: To class member ???
            // TODO: Place on the different CPU
            std::jthread thread(&TCPServer::eventsPoller, this);
            ...
            std::cout << std::format("Running on https://{}:{}", hostAddress, listenPort) << std::endl;
            while (true)
            {
                debug("Waiting for next connection ....");
                clientSocket = ::accept(serverSocket, reinterpret_cast<sockaddr*>(&clientAddr), &addLen);
    ...
    void startSerer()
    {
        TCPServer server {"0.0.0.0", 50001};
        if (server.createSockets())
            server.runServer();
    }
};
