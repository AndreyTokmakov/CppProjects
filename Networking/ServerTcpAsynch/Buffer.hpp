/**============================================================================
Name        : Buffer.hpp
Created on  : 12.08.2025
Author      : Andrei Tokmakov
Version     : 1.0
Copyright   : Your copyright notice
Description : Buffer.hpp
============================================================================**/

#ifndef CPPPROJECTS_BUFFER_HPP
#define CPPPROJECTS_BUFFER_HPP

#include <vector>
#include <cstdint>
#include <string>
#include <string_view>

namespace common
{
    class Buffer
    {
        using size_type = uint32_t;
        using data_type = uint8_t;
        using pointer   = data_type*;

        std::vector<data_type> buffer {};
        size_type length { 0 };

    public:

        explicit Buffer(const size_type size = 1024): buffer(size) {
        }

        void validateCapacity(const size_type size)
        {
            if (buffer.size() >= length + size)
                return;
            buffer.resize(length + size);
        }

        template<typename T = uint8_t>
        [[nodiscard]]
        T* data() noexcept {
            return reinterpret_cast<T*>(buffer.data());
        }

        template<typename T = uint8_t>
        [[nodiscard]]
        const T* data() const noexcept {
            return reinterpret_cast<T*>(buffer.data());
        }

        template<typename T = uint8_t>
        [[nodiscard]]
        T* head() noexcept {
            return reinterpret_cast<T*>(buffer.data()) + length;
        }

        template<typename T = uint8_t>
        [[nodiscard]]
        const T* head() const noexcept {
            return reinterpret_cast<T*>(buffer.data()) + length;
        }

        [[nodiscard]]
        size_type size() const noexcept {
            return length;
        }

        [[nodiscard]]
        std::string asString() const noexcept {
            return std::string { data<const char>(), length };
        }

        [[nodiscard]]
        std::string_view asStringView() const noexcept {
            return std::string_view { data<const char>(), length };
        }

        void reset() noexcept {
            length = 0;
        }

        void clear() noexcept {
            buffer.clear();
            length = 0;
        }

        void incrementLength(const size_type size) noexcept {
            length += size;
        }

        void swap(Buffer &buff) noexcept
        {
            std::swap(buff.buffer, buffer);
            std::swap(buff.length, length);
        }

        static void swap(Buffer &a, Buffer &b) noexcept
        {
            a.swap(b);
        }
    };

}

#endif //CPPPROJECTS_BUFFER_HPP
