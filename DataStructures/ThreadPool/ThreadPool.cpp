...
namespace ThreadPool
{
    ...
    struct Pool<ReturnType (Args...)>
    {
        ...
        static constexpr std::chrono::duration<uint64_t, std::ratio<1, 1000>> pollTimeout{
    ...
    public:
        ...
        explicit Pool(const uint32_t threadsCount = std::thread::hardware_concurrency())
        {
            workers.reserve(threadsCount);
            for (size_t i = 0; i < threadsCount; ++i) {
                workers.emplace_back(&Pool::executor, this, stopSource);
            }
        }
        ...
        template<typename Func, typename ... ParamTypes>
        Future submit(Func&& task, ParamTypes&& ... params) noexcept
        {
            std::unique_lock<std::mutex> lock { mutex };
            Future futureResult = taskQueue.emplace_back(std::forward<Func>(task),
                                                     std::forward<ParamTypes>(params)... ).task.get_future();
            lock.unlock();
